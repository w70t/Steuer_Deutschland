...
from bot.handlers.calculation import (
    ...
    receive_period,
    receive_state,
    receive_employment_type,
    receive_income,
    receive_tax_class,
    receive_children_has,
    receive_children_count,
    receive_kinderfreibetrag,
    receive_age_group,
    receive_health_insurance_type,
    receive_health_insurance_company,
    receive_church_tax,
    cancel_calculation,
    PERIOD,
    STATE,
...
async def main():
    ...
    calculation_conv = ConversationHandler(
        ...
        states={
            ...
            EMPLOYMENT_TYPE: [CallbackQueryHandler(receive_employment_type, pattern='^emp_')],
            ...
            CHILDREN_HAS: [CallbackQueryHandler(receive_children_has, pattern='^has_children_')],
            CHILDREN_COUNT: [CallbackQueryHandler(receive_children_count, pattern='^children_count_')],
            KINDERFREIBETRAG: [CallbackQueryHandler(receive_kinderfreibetrag, pattern='^kfb_')],
            AGE_GROUP: [CallbackQueryHandler(receive_age_group, pattern='^age_')],
            HEALTH_INSURANCE_TYPE: [CallbackQueryHandler(receive_health_insurance_type, pattern='^health_type_')],
            HEALTH_INSURANCE_COMPANY: [CallbackQueryHandler(receive_health_insurance_company, pattern='^hc_')],
